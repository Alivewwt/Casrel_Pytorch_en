...
class Framework(object):
    ...
    def test(self, test_data_loader, model, output=False, h_bar=0.20, t_bar=0.20):
        ...
        # while data is not None:
            with torch.no_grad():
                ...
                encoding = data_loader.tokenizer(text)
                ...
                if subjects:
                    ...
                    for subject_idx, subject in enumerate(subjects):
                        sub = subject[0]
                        h_start,_ = encoding.token_to_chars(subject[1])
                        _,t_end = encoding.token_to_chars(subject[2])
                        sub = text[h_start:t_end]
                        ...
                        for obj_head, rel_head in zip(*obj_heads):
                            for obj_tail, rel_tail in zip(*obj_tails):
                                if obj_head <= obj_tail and rel_head == rel_tail:
                                    ...
                                    o_start , _ = encoding.token_to_chars(obj_head)
                                    _, o_end = encoding.token_to_chars(obj_tail)
                                    obj = text[o_start:o_end]
